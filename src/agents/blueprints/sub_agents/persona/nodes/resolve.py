...
async def resolve_intent_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    llm: BaseChatModel | None = state.get("_llm")
    ...
    try:
        if llm is None:
            raise ValueError("LLM not configured in graph state")
        ...
        logger.exception("resolve_intent_node failed")
